...
`vendor/asdf.lisp` is **ASDF 3.3.7** (MIT, https://asdf.common-lisp.dev/). `SELECT lisp.asdf_version();` reads the file banner (`3.3.7`). `make install` bytecode-compiles `asdf.fasc` (LOAD then `compile-file` — ECL 26 bytecmp alone dies on `#.+max-character-type-index+`). `ensure-asdf` loads the fasc. Never `require` the distro image first.
