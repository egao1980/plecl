...
- Native CI: PGXS flags before `include $(PGXS)`; `push_macro` around `ecl.h` so PG `ERROR` survives; Homebrew `bdw-gc` include; skip PGDG LLVM bitcode; Windows `initdb --username`; packed systems eval `.lisp` (no `asdf:load-system` / `compile-file` in the backend); `lisp.asdf_version` reads the vendor banner; `asdf.fasc` via LOAD-then-compile-file (ECL 26 `#.+max-character-type-index+`); k-means++ seeded (`make-random-state 20260907`); Ubuntu `postgresql-16`; drop MSYS2 ECL job.
