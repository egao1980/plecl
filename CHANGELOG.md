...
- Native CI: PGXS flags before `include $(PGXS)`; `push_macro` around `ecl.h` so PG `ERROR` survives; Homebrew `bdw-gc` include; skip PGDG LLVM bitcode; Windows `initdb --username`; packed systems eval `.lisp` (no `asdf:load-system` / `compile-file` in the backend); `lisp.asdf_version` reads the vendor banner (eval of asdf.lisp SIGSEGVs Ubuntu ECL in PGDG postgres); Ubuntu `postgresql-16`; drop MSYS2 ECL job.
